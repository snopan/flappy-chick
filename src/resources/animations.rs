...
impl Animations {
    pub fn load(
        ...
        for (animation, sheet) in SPRITE_SHEETS {
            ...
            let layout = TextureAtlasLayout::from_grid(Vec2::new(sheet.tile_size, sheet.tile_size), sheet.columns, sheet.rows, None, None);
            let texture_atlas_layout = texture_atlas_layouts.add(layout);
            self.0.insert(animation, AnimationInfo{
                texture,
                atlas: TextureAtlas{
